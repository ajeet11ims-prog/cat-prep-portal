(function () {
  ...
  const ROOTS = {
    home: { label: "Dashboard", path: [], icon: "D", subtitle: "Choose a practice area and start in clean exam mode." },
    "topic-test": { label: "Topic Tests", path: ["Topic Test"], icon: "T", subtitle: "Topic-wise folders only. Open QA, LRDI, VARC and then the topic." },
    "previous-papers": { label: "Previous Papers", path: ["PYQ"], fallbackNames: ["Previous Papers", "Previous Paper", "Past Papers", "Past Year Papers"], icon: "P", subtitle: "Previous-year question practice in one place." },
    sectionals: { label: "Sectionals", path: ["Sectional Test"], fallbackNames: ["Sectionals", "Sectional"], icon: "S", subtitle: "Section-wise tests arranged folder by folder." },
    "full-length": { label: "Full Length", path: ["Full Length"], fallbackNames: ["Full Test", "Full Length Test", "Mock Test", "Mocks"], icon: "F", subtitle: "Complete mocks and full-length practice papers." }
  };
  ...
  function routeSlugForPath(path) {
    const key = Object.keys(ROOTS).find(routeKey => samePath(routeRootPath(routeKey), path));
    return key || pathSlug(path);
  }
  ...
  function pathFromHash() {
    ...
    // Support earlier URLs like #folder/topic-test-qa-arithmetic-2-ratio-proportion
    ...
    if (routeHit) {
      ...
      const rootSlug = slug(ROOTS[routeHit].path[0] || routeHit);
      const possible = keys
        .map(key => key.split(" / "))
      ...
      const best = possible.find(path => wanted === pathSlug(path) || wanted.endsWith(pathSlug(path).replace(rootSlug + "-", "")))
        || possible.find(path => pathSlug(path).includes(wanted.replace(routeHit + "-", "")));
      if (best) return { route: routeHit, path: best };
      return { route: routeHit, path: rootPath };
    }

    // Old CAT QA route support.
    if (wanted.startsWith("topic-qa")) {
      const translated = wanted.replace(/^topic-qa/, "topic-test-qa");
      const match = allFolderKeys().find(key => slug(key).includes(translated.replace("topic-test-", "topic-test-")) || slug(key).endsWith(translated.replace("topic-test-", "")));
  ...
  function setHash(path) {
    ...
    history.replaceState(null, "", `#folder/${pathSlug(path)}`);
  }
  ...
  function cleanFolderName(name) {
    return String(name || "Folder").replace(/^\d+\.?\s*/, "").replace(/\s+/g, " ").trim();
  }
    ...
    return `
      <a class="folder-card" href="#folder/${pathSlug(folder.path)}">
        <div>
          ...
          <p class="card-desc">${escapeHtml(folder.path.join(" / "))}</p>
        </div>
  ...
  function renderDashboard() {
    ...
    updateHeader("Dashboard", "Topic Tests, Previous Papers, Sectionals and Full Length are separated in the left navigation.");
    els.breadcrumb.textContent = "Dashboard";
    els.contentTitle.textContent = "Browse Tests";
    els.contentNote.textContent = "Open Topic Tests to see only topic-test folders first.";
    els.dashboardView.innerHTML = `<div class="grid">${["topic-test", "previous-papers", "sectionals", "full-length"].map(dashboardCard).join("")}</div>`;
  }
  ...
  function renderPath(path, routeKey) {
    ...
    els.breadcrumb.textContent = path.length ? path.join(" / ") : "Dashboard";
  ...
  function testCard(test) {
    ...
    if (test.questions) tags.push(`${test.questions} questions`);
