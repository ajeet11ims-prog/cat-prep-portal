(function () {
  ...
  function cardIcon(name) {
    ...
    if (lower.includes("average")) return "A";
  ...
  function helperStrip() {
    return `
      <div class="helper-strip">
        <div class="helper-card"><strong>1. Choose section</strong><span>Topic Tests, PYQs, Sectionals or Full Length.</span></div>
  ...
  function renderDashboard() {
    ...
    updateHeader("CAT-MBA Prep Zone", "A calm test dashboard for topic tests, previous papers, sectionals and full-length practice.");
    ...
    els.dashboardView.innerHTML = `${helperStrip()}<div class="grid">${["topic-test", "previous-papers", "sectionals", "full-length"].map(dashboardCard).join("")}</div>`;
  }
